        ...
        {foreach $block_blog_entries as $one_blog_entry}
            <div class="media">
                {if !empty($one_blog_entry.image)}
                    <a href="{$smarty.const.IA_URL}blog/{$one_blog_entry.id}-{$one_blog_entry.alias}" class="media-object pull-left">{ia_image file=$one_blog_entry.image width=40 title=$one_blog_entry.title|escape class='img-responsive'}</a>
                {/if}
