<div class="b-card b-card--border">
    <div class="b-card__image">
        <a href="{$venue.link}">
            {ia_image file=$venue.image title=$venue.venue_title class='img-responsive' type='thumbnail'}
        </a>

        {printFavorites item=$venue itemtype=$venue.item guests=true tpl='no-text' classname='b-card__fav'}
        {if 'yellowpages_home' != $core.page.name && isset($venue.category_title) && '' != $venue.category_title}
            <a class="b-card__cat" href="{ia_url item='ycateg' type='url' data=$venue action='view'}" data-toggle="tooltip" title="{$venue.category_title}"><span class="fa fa-{if $venue.category_icon_class}{$venue.category_icon_class}{else}folder{/if}"></span></a>
        {/if}
    </div>
    ...
    <div class="b-card__content">
        ...
        <div class="b-card__title">{ia_url type='link' item=$venue.item data=$venue text=$venue.venue_title}</div>
        <p><span class="fa fa-map-marker"></span> {$venue.address}, {ia_url item='yloc' type='link' data=$venue text=$venue.location}{if !empty($venue.zipcode)} {$venue.zipcode}{/if}</p>
    </div>
</div>