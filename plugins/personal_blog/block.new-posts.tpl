		...
		{foreach $block_blog_entries as $one_blog_entry}
			<div class="media">
				{if $one_blog_entry.image}
					<a href="{$smarty.const.IA_URL}blog/{$one_blog_entry.id}-{$one_blog_entry.alias}" class="media-object pull-left">{printImage imgfile=$one_blog_entry.image width='40' title=$one_blog_entry.title|escape: 'html' class='img-responsive'}</a>
				{/if}
				<div class="media-body">
					<h5 class="media-heading">
						<a href="{$smarty.const.IA_URL}blog/{$one_blog_entry.id}-{$one_blog_entry.alias}">{$one_blog_entry.title|escape: 'html'}</a>
					</h5>
					<p>{$one_blog_entry.date_added|date_format:$core.config.date_format} {lang key='by'} {$one_blog_entry.fullname}</p>
				</div>
			</div>
